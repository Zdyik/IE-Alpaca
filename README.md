...
[V11自监督表征学习](docs/V11自监督表征学习设计.md)已完成实现、五折主实验和三随机种子复验。四种方法共用39,517参数的因果编码器和相同风险微调协议；严格OOF中，每折预训练、缩放和选轮只读取该折训练车辆，96台锁定车仍未参与训练或评分。

首个种子的20→40开发OOF AUC为：Scratch **0.7289**、事件感知Masked Reconstruction **0.7743**、Contrastive **0.7447**、JEPA **0.7232**。Masked相对同架构Scratch提升 **+0.0454**，配对bootstrap区间为`[+0.0131,+0.0802]`，Brier由 **0.1872**改善至 **0.1736**；4/5折提升，14→40也同步改善。Masked在2027/2028种子的AUC为 **0.7707/0.7704**，三种子均值 **0.7718 ± 0.0022**，相对各自Scratch的平均提升 **+0.0429 ± 0.0039**。

三种子逐车概率集成的20→40 AUC为 **0.7741**、Brier **0.1733**。它高于V10 Chain-Aux **+0.0112**，但配对区间跨0；低于V3 Model 0 **−0.0134**，区间也跨0，因此V11证明了Masked预训练对这套深度编码器有效，但尚未证明总体优于V3。最终V11运行为`V11ENS_20260926_090415_be888624`，500车三列交付表位于该数据库运行目录的`deliverables/task1_v11_predictions.csv`。

```powershell
python train_v11.py --config configs/experiments/v11_stage1.json
python train_v11.py --config configs/experiments/v11_masked_seed2027.json
python train_v11.py --config configs/experiments/v11_masked_seed2028.json
python aggregate_v11.py
```

## 当前进展：V10 深度风险链网络

[V10深度风险链网络](docs/V10深度风险链网络实验设计.md)的六个预注册变体已经全部实现和训练。V10不使用V3汇总特征或逻辑回归权重，而是从24类事件和暴露、IMU、质量组成的每日节点序列独立训练。人的判断进入事件角色embedding、1–7天方向图、慢性/急性/升级池化和DMS质量门控；事件作用、图边强度和风险率由网络学习。

第一阶段相同40轮预算下，Flat、Role、Chain、同参数乱序图的20→40开发OOF AUC分别为 **0.7331 / 0.7578 / 0.7611 / 0.7591**。合理风险链只比角色网络高 **0.0033**、比乱序图高 **0.0020**，配对区间均跨0，因此不能声称图拓扑已经得到统计证实。折内选轮后，加入七日中间阶段监督的Chain-Aux为 **0.7629**，比同协议Chain高 **0.0086**；质量门控版为 **0.7612**，没有进一步改善主检查点。V10最终保留Chain-Aux，但V3 Model 0的 **0.7875** 仍是开发集首选。

```powershell
python train_v10.py --config configs/experiments/v10_stage1.json
python train_v10.py --config configs/experiments/v10_stage2_aux.json
python train_v10.py --config configs/experiments/v10_stage3_quality.json
```

三个完成运行是`V10S1_20260923_145411_d9aec959`、`V10S2_20260923_151111_fc2709b4`和`V10S3_20260923_152406_40e98aaf`。最终V10候选及事件遮蔽分析位于第二个运行目录；三列交付文件为`deliverables/task1_v10_predictions.csv`。96台锁定车仍未评分，Day60→未来40天没有本地真值。
