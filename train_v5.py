"""V5: choose V4 training epochs inside each outer fold's training vehicles."""
...
def run(config_path: Path, database_root: Path) -> Path:
    ...
    if tuple(config["anchors"]) != ANCHORS or config["threshold"] != 0.5:
        raise ValueError("V5 锚点与分类阈值必须与 V4 一致")
    ...
    if not at20.eq(reference.set_index("gpsno").loc[at20.index, "label"]).all():
        raise ValueError("V5 的 20→40 标签与 V4/V3 不一致")
    ...
    run_dir, manifest = create_run(REPO, results_root, config_path, resolved,
                                   version="V5", entrypoint="train_v5.py")
    try:
        manifest.update({"model": "torch_all_event_stationary_hazard_v5_epoch_selection", "split_version": split["version"],
                         "data_files": data_files, "data_fingerprint": fingerprint,
                         "evaluation_version": "v5_vehicle_oof_stationary_hazard_inner_epoch_selection_0.5",
        ...
        metrics = {"target": "day-end landmark next-event hazard; right censored at day 60",
                   ...
                   "change_from_v4": "vehicle-isolated inner epoch selection; V4 inputs, network and loss unchanged",
        ...
        v4 = read_v3_reference(results_root, config["parent_run_id"])
        metrics["paired_vs_v4_20_to_40"] = paired_auc(current, v4, seed=config["seed"])
        ...
        delta = metrics["paired_vs_v4_20_to_40"]["delta_auc"]
        (run_dir / "result_summary.md").write_text(
            f"# {manifest['run_id']}\n\n"
            f"- 20→40 learned-weight OOF AUC: {score['roc_auc']:.6f}; 对 V4 ΔAUC: {delta:+.6f}\n"
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=REPO / "configs/experiments/v5_epoch_selection.json")
