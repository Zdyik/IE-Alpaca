"""Train all-event PyTorch hazard and a uniform-weight control on frozen vehicle folds."""
...
def run(config_path: Path, database_root: Path) -> Path:
    ...
    if tuple(config["anchors"]) != ANCHORS or config["threshold"] != 0.5:
        raise ValueError("V4 首轮锚点与分类阈值必须与 V3 一致")
    ...
    if not at20.eq(reference.set_index("gpsno").loc[at20.index, "label"]).all():
        raise ValueError("V4 的 20→40 标签与 V3/V1 不一致")
    ...
    run_dir, manifest = create_run(REPO, results_root, config_path, resolved,
                                   version="V4", entrypoint="train_v4.py")
    try:
        manifest.update({"model": "torch_all_event_stationary_hazard", "split_version": split["version"],
                         "data_files": data_files, "data_fingerprint": fingerprint,
                         "evaluation_version": "v4_vehicle_oof_stationary_hazard_fixed_threshold_0.5",
        ...
        for fold_text, val_list in sorted(split["validation_folds"].items(), key=lambda x: int(x[0])):
            ...
            for variant, learned in (("uniform", False), ("learned", True)):
                net, history = fit_network(pack, config["model"], learned_weights=learned,
        ...
        v3 = read_v3_reference(results_root, config["parent_run_id"])
        ...
        delta = metrics["paired_vs_v3_20_to_40"]["delta_auc"]
        (run_dir / "result_summary.md").write_text(
            f"# {manifest['run_id']}\n\n"
            f"- 20→40 learned-weight OOF AUC: {score['roc_auc']:.6f}; 对 V3 ΔAUC: {delta:+.6f}\n"
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=REPO / "configs/experiments/v4_event_hazard.json")
