"""Export one completed V8 run as gpsno,prediction,probability."""
...
def export(run_dir: Path, profile_path: Path, output: Path | None = None) -> Path:
    ...
    if manifest.get("status") != "completed" or not str(manifest.get("model", "")).endswith("_hazard_v8"):
        raise ValueError("Only a completed V8 hazard run may be exported")
    source = _read(
    ...
    if len(source) != 500 or source.gpsno.isna().any() or source.gpsno.duplicated().any():
        raise ValueError("V8 candidates must cover 500 unique vehicles")
    ...
    pred = source.prediction_at_0_5.to_numpy(dtype=int)
    if not np.isfinite(p).all() or ((p < 0) | (p > 1)).any():
        raise ValueError("Probabilities must be finite and between 0 and 1")
    if not np.array_equal(pred, (p >= .5).astype(int)):
    ...
    suffix = "v8a" if manifest["architecture"] == "deep_additive" else "v8b"
    output = output.resolve() if output else run_dir / "deliverables" / f"task1_{suffix}_predictions.csv"
    if not output.is_relative_to(run_dir):
        raise ValueError("Delivery must stay inside its run directory")
    ...
    shutil.copy2(Path(__file__), run_dir / "source" / "export_task1_v8.py")
    return output
