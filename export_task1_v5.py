"""Export one completed V5 run in the same three-column format as V4."""
...
def export(run_dir: Path, profile_path: Path, output: Path | None = None) -> Path:
    ...
    if manifest.get("status") != "completed" or manifest.get("model") != "torch_all_event_stationary_hazard_v5_epoch_selection":
        raise ValueError("只能导出已完成的 V5 全事件风险模型")
    source = _read(
    ...
    if source.route.eq("prior_no_observed_behavior").sum() != 22:
        raise ValueError("低证据车辆数与 V5 运行记录不一致")

    output = output.resolve() if output else run_dir / "deliverables" / "task1_v5_predictions.csv"
    ...
    shutil.copy2(Path(__file__), run_dir / "source" / "export_task1_v5.py")
    return output
